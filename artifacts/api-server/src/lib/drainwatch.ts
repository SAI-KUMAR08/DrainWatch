  ...
  const seed = [
    {
      ...
      latitude: 17.4239,
      longitude: 78.4738,
      locationName: "Tank Bund Road, Hyderabad",
      description: "Water has reached the edge of the carriageway near the public promenade.",
      status: "verified",
    ...
    },
    {
      ...
      description: "Drain grate is blocked with silt and plastic after overnight rain.",
      status: "assigned",
    ...
    },
    {
      ...
      latitude: 17.4948,
      longitude: 78.3996,
      locationName: "Kukatpally Housing Board",
      description: "Open manhole reported beside the service road near the bus stop.",
      status: "in_progress",
      verificationStatus: "officer_verified",
      source: "citizen",
    },
    {
      ...
      latitude: 17.385,
      longitude: 78.4867,
      locationName: "Mehdipatnam Ring Road",
      description: "Overflow is affecting the pedestrian lane outside the market entrance.",
      status: "reported",
      verificationStatus: "pending_review",
      source: "news",
    },
  ...
  await db.insert(reportsTable).values(
    seed.map((item, index) => {
      ...
      return {
        ...
        isDemo: true,
        createdAt: new Date(now.getTime() - index * 75 * 60 * 1000),
        updatedAt: now,
      };
    }),
  );
...
export async function insertCitizenReport(input: {
  ...
  const severity: Severity =
    input.hazard_type === "open_manhole" ? "high" : input.hazard_type === "waterlogging" ? "medium" : "low";
...
export async function getDashboardStats() {
  ...
  return {
    active_reports: reports.filter((report) => !["resolved", "rejected"].includes(report.status)).length,
    critical_reports: reports.filter((report) => report.riskScore >= 75).length,
    verified_today: reports.filter((report) => report.verificationStatus === "officer_verified" && report.updatedAt >= today).length,
    resolved_today: reports.filter((report) => report.status === "resolved" && report.updatedAt >= today).length,
    total_reports: reports.length,
    rainfall_status: "unavailable",
    is_demo: reports.some((report) => report.isDemo),
  };
}
